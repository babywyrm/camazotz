...
def test_gateway_calls_auth_tool() -> None:
    client = TestClient(app)
    result = tool_call(client, "auth.issue_token", {"username": "alice"}, 11)
...
def test_gateway_calls_context_tool_returns_summary() -> None:
    client = TestClient(app)
    result = tool_call(client, "context.injectable_summary", {"text": "This is a test document."}, 16)
...
def test_gateway_calls_tool_mutation_before_threshold() -> None:
    client = TestClient(app)
    result = tool_call(client, "tool.mutate_behavior", {"mode": "status"}, 12)
...
def test_gateway_tool_rug_pull_after_threshold() -> None:
    client = TestClient(app)
    for i in range(7):
        rpc_call(client, "tools/call", {"name": "tool.mutate_behavior", "arguments": {}}, 100 + i)

    body = rpc_call(client, "tools/list", {}, 200)
    ...
    with patch("camazotz_modules.tool_lab.app.main.subprocess.run") as mock_run:
        ...
        result = tool_call(client, "tool.hidden_exec", {"command": "id"}, 201)
    assert result["_real_output"] is True


def test_gateway_hidden_exec_before_threshold() -> None:
    client = TestClient(app)
    body = rpc_call(client, "tools/call", {"name": "tool.hidden_exec", "arguments": {"command": "id"}}, 300)
...
def test_gateway_returns_error_for_unknown_tool() -> None:
    client = TestClient(app)
    body = rpc_call(client, "tools/call", {"name": "tool.unknown", "arguments": {}}, 99)
...
def test_secrets_leak_config_returns_all_creds() -> None:
    client = TestClient(app)
    result = tool_call(client, "secrets.leak_config", {}, 400)
...
def test_secrets_leak_config_with_filter() -> None:
    client = TestClient(app)
    result = tool_call(client, "secrets.leak_config", {"filter": "AWS"}, 401)
...
def test_shadow_register_webhook() -> None:
    client = TestClient(app)
    result = tool_call(client, "shadow.register_webhook", {
...
def test_shadow_list_webhooks_after_registration() -> None:
    client = TestClient(app)
    tool_call(client, "shadow.register_webhook", {"url": "https://evil.com/hook1", "label": "hook1"}, 610)
    tool_call(client, "shadow.register_webhook", {"url": "https://evil.com/hook2", "label": "hook2"}, 611)
    result = tool_call(client, "shadow.list_webhooks", {}, 620)
    assert result["count"] == 2
...
def test_shadow_list_webhooks_empty() -> None:
    client = TestClient(app)
    result = tool_call(client, "shadow.list_webhooks", {}, 630)
    assert result["count"] == 0
...
def test_auth_access_protected_valid_token() -> None:
    client = TestClient(app)
    issue = tool_call(client, "auth.issue_token", {"username": "tester"}, 700)
    token = issue["token"]
    result = tool_call(client, "auth.access_protected", {"token": token, "resource": "config"}, 701)
    assert result["access"] == "granted"


def test_auth_access_protected_invalid_token() -> None:
    client = TestClient(app)
    result = tool_call(client, "auth.access_protected", {"token": "bogus-token", "resource": "config"}, 702)
    assert result["access"] == "denied"


def test_auth_access_protected_insufficient_role() -> None:
    client = TestClient(app)
    issue = tool_call(client, "auth.issue_token", {"username": "lowpriv"}, 703)
    token = issue["token"]
    if "reader" in token:
        result = tool_call(client, "auth.access_protected", {"token": token, "resource": "admin-panel"}, 704)
        assert result["access"] == "denied"
